...
/// <summary>
/// Budget controller — same route/actor/exception-mapping pattern as <see cref="JournalEntriesController"/>.
/// No Update/Delete endpoint: a Budget's Amount is fixed once created — see <see cref="Domain.Budget"/>'s
/// own doc comment for why a revision means Reject-then-create-new rather than editing in place.
/// </summary>
[Route("api/v1/finance/budgets")]
public sealed class BudgetsController : PlatformApiController
{
    ...
    public async Task<IActionResult> List(CancellationToken cancellationToken)
    {
        ...
        return Ok(new PagedResult<BudgetDto>(items, totalCount, query.Skip, query.Top));
    }

    [HttpGet("{id:guid}")]
    public async Task<IActionResult> Get(Guid id, CancellationToken cancellationToken)
    {
        var budget = await _service.GetAsync(id, cancellationToken);
