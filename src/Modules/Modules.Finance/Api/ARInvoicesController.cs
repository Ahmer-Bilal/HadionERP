...
public sealed record ReverseARInvoiceRequest(DateOnly? ReversalDate = null);

/// <summary>
/// AR Invoice controller — inherits <see cref="PlatformApiController"/> for shared conventions, same
/// route/actor/exception-mapping pattern as <see cref="APInvoicesController"/>.
/// </summary>
[Route("api/v1/finance/ar-invoices")]
public sealed class ARInvoicesController : PlatformApiController
{
    private readonly ARInvoiceService _service;

    public ARInvoicesController(ARInvoiceService service) => _service = service;
    ...
    public async Task<IActionResult> List(CancellationToken cancellationToken)
    {
        ...
        return Ok(new PagedResult<ARInvoiceDto>(items, totalCount, query.Skip, query.Top));
    }
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreateARInvoiceRequest request, CancellationToken cancellationToken)
    {
    ...
    public async Task<IActionResult> Reverse(Guid id, [FromBody] ReverseARInvoiceRequest? request, CancellationToken cancellationToken)
    {
