...
public sealed record ReverseCustomerReceiptRequest(DateOnly? ReversalDate = null);

/// <summary>
/// Customer Receipt controller — the AR mirror of <see cref="PaymentsController"/>, same conventions.
/// </summary>
[Route("api/v1/finance/customer-receipts")]
public sealed class CustomerReceiptsController : PlatformApiController
{
    private readonly CustomerReceiptService _service;

    public CustomerReceiptsController(CustomerReceiptService service) => _service = service;

    [HttpGet]
    public async Task<IActionResult> List([FromQuery] Guid? arInvoiceId, CancellationToken cancellationToken)
    {
        if (arInvoiceId is { } invoiceId)
        {
            var forInvoice = await _service.ListByInvoiceAsync(invoiceId, cancellationToken);
            return Ok(new PagedResult<CustomerReceiptDto>(forInvoice, forInvoice.Count, 0, forInvoice.Count));
        }
        ...
        return Ok(new PagedResult<CustomerReceiptDto>(items, totalCount, query.Skip, query.Top));
    }

    [HttpGet("{id:guid}")]
    public async Task<IActionResult> Get(Guid id, CancellationToken cancellationToken)
    {
        var receipt = await _service.GetAsync(id, cancellationToken);
        return receipt is null ? NotFound() : Ok(receipt);
    }

    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreateCustomerReceiptRequest request, CancellationToken cancellationToken)
    {
    ...
    public async Task<IActionResult> Reverse(Guid id, [FromBody] ReverseCustomerReceiptRequest? request, CancellationToken cancellationToken)
    {
