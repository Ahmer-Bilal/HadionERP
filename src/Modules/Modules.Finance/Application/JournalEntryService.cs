...
public sealed class JournalEntryService
{
    ...
    public async Task<JournalEntryDto> ReverseAsync(Guid id, string actor, DateOnly reversalDate, CancellationToken cancellationToken = default)
    {
        ...
        var mirrorLines = original.Lines
            .Select(line => new CreateJournalLineRequest(line.GLAccountId, line.CreditAmount, line.DebitAmount, line.CostCenterId, line.LineDescription))
            .ToList();
    ...
    /// <summary>
    /// Creates a Journal Entry whose content is mechanically derived by Finance itself — a reversal
    /// mirror (see <see cref="ReverseAsync"/>) or a linked posting generated by another Finance document
    /// (e.g. <c>APInvoiceService.PostAsync</c>) — and drives it straight to Posted, bypassing the human
    /// approval workflow real new entries go through via <see cref="SubmitAsync"/>. Still validates every
    /// line's G/L Account/Cost Center reference the same way a real entry does; only the approval step is
    /// skipped, and only because the content isn't new human judgment. Public so other Finance services
    /// can reuse it without duplicating this "construct → validate lines → number → drive lifecycle"
    /// sequence — same rationale as everywhere else in this codebase for extracting a second real
    /// consumer's shared logic instead of copy-pasting it.
    /// </summary>
        ...
        Guid? reversalOfEntryId, string actor, CancellationToken cancellationToken = default)
    {
        ...
        _auditRecorder.RecordCreate(AuditReference(entry.Id), actor,
            $"Journal entry '{entry.DocumentNumber}' created (system-generated).", AuditSource);
