...
/// <summary>
/// Contract controller — inherits <see cref="PlatformApiController"/> for shared conventions, same
/// route/actor/exception-mapping pattern as every other module's controller.
/// </summary>
[Route("api/v1/construction/contracts")]
public sealed class ContractsController : PlatformApiController
{
    ...
    public async Task<IActionResult> List(CancellationToken cancellationToken)
    {
        ...
        return Ok(new PagedResult<ContractDto>(items, totalCount, query.Skip, query.Top));
    }

    [HttpGet("{id:guid}")]
    public async Task<IActionResult> Get(Guid id, CancellationToken cancellationToken)
    {
        var contract = await _service.GetAsync(id, cancellationToken);
