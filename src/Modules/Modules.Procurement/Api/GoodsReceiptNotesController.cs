...
/// <summary>
/// Goods Receipt Note controller — inherits <see cref="PlatformApiController"/> for shared conventions, same
/// route/actor/exception-mapping pattern as every other module's controller.
/// </summary>
[Route("api/v1/procurement/goods-receipt-notes")]
public sealed class GoodsReceiptNotesController : PlatformApiController
{
    private readonly GoodsReceiptNoteService _service;

    public GoodsReceiptNotesController(GoodsReceiptNoteService service) => _service = service;
    ...
    public async Task<IActionResult> List(CancellationToken cancellationToken)
    {
        ...
        return Ok(new PagedResult<GoodsReceiptNoteDto>(items, totalCount, query.Skip, query.Top));
    }

    [HttpGet("{id:guid}")]
    public async Task<IActionResult> Get(Guid id, CancellationToken cancellationToken)
    {
        var grn = await _service.GetAsync(id, cancellationToken);
        return grn is null ? NotFound() : Ok(grn);
    }

    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreateGoodsReceiptNoteRequest request, CancellationToken cancellationToken)
    {
