...
/// <summary>
/// Purchase Requisition controller — inherits <see cref="PlatformApiController"/> for shared conventions,
/// same route/actor/exception-mapping pattern as every other module's controller.
/// </summary>
[Route("api/v1/procurement/purchase-requisitions")]
public sealed class PurchaseRequisitionsController : PlatformApiController
{
    private const string MaintainerActor = "system/ui";
    private const string ApproverActor = "system/approver";

    private readonly PurchaseRequisitionService _service;

    public PurchaseRequisitionsController(PurchaseRequisitionService service) => _service = service;
    ...
    public async Task<IActionResult> List(CancellationToken cancellationToken)
    {
        ...
        return Ok(new PagedResult<PurchaseRequisitionDto>(items, totalCount, query.Skip, query.Top));
    }

    [HttpGet("{id:guid}")]
    public async Task<IActionResult> Get(Guid id, CancellationToken cancellationToken)
    {
        var requisition = await _service.GetAsync(id, cancellationToken);
        return requisition is null ? NotFound() : Ok(requisition);
    }

    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreatePurchaseRequisitionRequest request, CancellationToken cancellationToken)
    {
